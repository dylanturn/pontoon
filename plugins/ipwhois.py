...
cache_client = cache.RedisCache(os.environ['REDIS_HOST'], os.environ['REDIS_PORT'], os.environ['REDIS_DB'])


def lookup_ip(ip_address):
  # Try get the ip data from the cache
  ip_data = cache_client.get_cache_item(ip_address)
  # If the entry expired then we'll resolve it
  if ip_data is None:
    ip_data = resolve_ip(ip_address)
    # Cache the response if we were able to resolve the IP.
    if ip_data is not None:
      cache_client.cache_item(ip_address, ip_data)
...
@plugin.route('/<ip_address>', methods=['GET', 'PURGE'])
def ipwhois_ip_address(ip_address=None):
  if request.method == 'GET':
    ip_data = lookup_ip(ip_address)
    if ip_data is None:
        return make_response({}, 404)
    else:
        return ip_data
  if request.method == 'PURGE':
    cache_client.expire_cache_item(ip_address)
